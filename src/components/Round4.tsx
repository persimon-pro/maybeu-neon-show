...
export function Round4({ gameState }: Round4Props) {
  ...
  // Preload all photos in round 4 immediately
  useEffect(() => {
  ...
  if (roundEnd || !currentItem) {
    return (
      <div className="w-full h-full p-4 flex flex-col items-center justify-center text-center">
        <h2 className="text-2xl sm:text-4xl md:text-6xl font-black text-amber-500 drop-shadow-[0_0_20px_rgba(245,158,11,0.8)] mb-4 uppercase tracking-widest text-center">
  ...
  return (
    <div className="w-full h-full p-2 sm:p-4 md:p-5 flex flex-col items-center justify-between overflow-hidden max-w-7xl mx-auto">
      ...
      <div className="text-center shrink-0 my-1">
        <h1 className="text-base sm:text-xl md:text-2xl font-bold uppercase tracking-[0.15em] text-amber-500 drop-shadow-[0_0_15px_rgba(245,158,11,0.5)]">
          // {round.title}
        </h1>
      </div>
        ...
        <div className="w-full max-w-4xl h-full max-h-[75vh] glass-panel rounded-2xl sm:rounded-3xl p-3 sm:p-5 flex flex-col items-center justify-between relative overflow-hidden shadow-2xl">
          ...
          <div className="relative flex-1 min-h-0 w-full rounded-xl sm:rounded-2xl overflow-hidden border border-white/10 bg-black/60 flex items-center justify-center">
            <img 
              ...
              className={`w-auto h-auto max-w-full max-h-full object-contain select-none transition-all ${isAnswerRevealed ? 'opacity-35 grayscale' : ''}`}
            />
            {/* Overlay for answer */}
            {isAnswerRevealed && (
              <div className="absolute inset-0 bg-slate-950/85 backdrop-blur-md flex flex-col items-center justify-center p-4 sm:p-8 z-20 animate-fade-in">
                <h2 className="text-xl sm:text-3xl md:text-5xl lg:text-6xl text-center font-black text-amber-400 uppercase tracking-wider leading-tight drop-shadow-[0_0_20px_rgba(245,158,11,0.8)]">
          ...
          <div className="shrink-0 mt-2 text-[9px] sm:text-xs text-amber-500/80 font-mono tracking-[0.2em] uppercase z-10 flex items-center">
             АРТЕФАКТ {itemIndex + 1} ИЗ {round.items.length} <span className="text-white/30 mx-2">//</span> HISTORICAL_SCAN_ACTIVE
          </div>
        </div>
      </div>
